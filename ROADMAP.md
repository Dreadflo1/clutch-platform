...
- [ ] **Dispute (vrai litige win/win)** : statuts typés + auto-refund au timeout en place, mais il manque la **résolution active** (file admin / re-verify manuel) pour trancher un gagnant au lieu d'un simple remboursement.
- [ ] Sortir la vérif réseau **hors du lock** (G) : aujourd'hui `resolveOutcome` tourne dans le lock 15s → risque d'expiration si Riot est lent.
